...
def main():
    ...
    try:
        ...
        postgisimport(file_path, db_name, collection_name, 5433)
    except Exception as e:
        print(str(e))
        print("No such file or directory.")


def postgisimport(csv_path, db_name, table_name, db_port=5432):
    geolocator = Nominatim(
        user_agent="geocoder_intento2")
    ...
    for i, row in df.iterrows():
        if(i % 500 == 0):
            conn.commit()
            print("nice")
        if not math.isnan(row.Lat) != "nan" and not math.isnan(row.Long) != "nan":
